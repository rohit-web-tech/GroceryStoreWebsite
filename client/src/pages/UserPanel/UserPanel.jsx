...
const UserPanel = ({ setLoggedIn }) => {
    let loggedInUser = JSON.parse(localStorage.getItem('srpcuser'));
    loggedInUser={...loggedInUser,userAddress: loggedInUser.userAddress || ""};
            ...
            <div id="user-panel">
                ...
                <Tabs defaultActiveKey="1">
                    <TabPane tab="My Profile" key="1">
                        <UserProfile setLoggedIn={setLoggedIn} userDetails={userDetails} setUserDetails={setUserDetails}/>
                    </TabPane>
...
export function UserProfile({ setLoggedIn,userDetails,setUserDetails}) {
    ...
    const handleSaveChanges =()=>{
        if(userDetails.userName&&userDetails.userContact&&userDetails.userAddress&&userDetails.userEmail){
        ...
        <div id="user-profile">
            ...
            <form>
                <div className="row">
                    ...
                    <input type="text" name="userName" id="username" value={userDetails?.userName} disabled={editProfile} onChange={handleUserInput}/>
                </div>
                <div className="row">
                    <label htmlFor="name">Number : </label>
                    <input type="text" name="userContact" id="username" value={userDetails?.userContact} disabled={editProfile} onChange={handleUserInput}/>
                </div>
                <div className="row">
                    <label htmlFor="name">Email Id : </label>
                    <input type="text" name="userEmail" id="username" value={userDetails?.userEmail} disabled={editProfile} onClick={handleEmailChange} onInput={handleEmailChange}/>
                </div>
                <div className="row">
                    <label htmlFor="name">Address : </label>
                    <input type="text" name="userAddress" id="username" value={userDetails?.userAddress} disabled={editProfile} onChange={handleUserInput}/>
                </div>
            </form>
            <div className="btns">
                {
                    editProfile ? (
                        <>
                        <button className="log-out-btn btn" onClick={handleLogOut}><i class="fa-solid fa-right-from-bracket"></i> Log Out</button>
                        <button className="edit-btn btn" onClick={()=>{setEditProfile(false)}}><i class="fa-solid fa-pen-to-square"></i> Edit Profile</button>
                        </>
                    ):(
                        <>
                        <button className="log-out-btn btn" onClick={handleCancel}><i class="fa-solid fa-close"></i> Cancel</button>
                        <button className='save-profile btn' onClick={handleSaveChanges}><i class="fa-regular fa-floppy-disk"></i> Save Changes</button>
                        </>
                    )
                }
            </div>
...
export function MyOrders() {
    ...
    useEffect(() => {
        fetchPostDataFromApi('/getOrders', { userId })
            .then(res => setOrderItems(res.orders))
                                    ...
                                    <tr className='table-row flex-box align-center'>
                                        <td className='image-item-column flex-box justify-center'>
                                            <div class="product-image">
                                                <Img src={orderItem?.productImg} alt="" className='lazy-img' />
                                            </div>
                                        </td>
                                        ...
                                        <td className='subtotal-item-column'>
                                            <div class="product-subtotal">
                                                <p>{orderItem?.productQuantity + " " + orderItem?.productUnit}</p>
                                            </div>
                                        </td>
                                        <td className='subtotal-item-column'>
                                            <div class="product-subtotal">
